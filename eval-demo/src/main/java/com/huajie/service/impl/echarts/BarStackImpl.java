...
//        return getLegendStack(parameters.get("teamName").toString());
    ...
    public void getExpMap(String params, Map<String, List<ExtMapData<String, String>>> mapResult) {
        List<ExtMapData<String, String>> emd = new ArrayList<>();
        List<Map<String, Object>> maps = jdbcTemplate.queryForList("select update_date, expect_day_work from `jindu` where station = '" + params + "'");
        for (Map<String, Object> map : maps) {
            emd.add(new ExtMapData(map.get("update_date").toString(), map.get("expect_day_work").toString()));
...
//            }
    ...
    public void getActMap(String params, Map<String, List<ExtMapData<String, String>>> mapResult) {
        List<ExtMapData<String, String>> emd = new ArrayList<>();
        List<Map<String, Object>> maps = jdbcTemplate.queryForList("select update_date, actual_today_work from `jindu` where station = '" + params + "'");
        for (Map<String, Object> map : maps) {
...
//            }
    ...
    protected Map<String, List<ExtMapData<String, String>>> getSeriesData() {
        ...
        getExpMap(parameters.get("teamName").toString(), map);
        getActMap(parameters.get("teamName").toString(), map);
