...
EchartsTool.prototype = (function () {
    return {
            ...
            var option = {
                ...
                label: {
                    ...
                    formatter: function (params) {
                        ...
                        var goodsValue = data['value'];
                        return goodsName + ':' + goodsValue + "个";
                    }
                },
                tooltip: {
                    ...
                    function (params) {
                        ...
                        var goodsName = data['name'];
                        var goodsValue = data['value'];
                        return "消耗量<br/>" + goodsName + ':' + goodsValue + "个";
                    }
                },
                legend: {
                    orient: 'vertical',
        ...
        initBarStack: function (conf, param) {
            ...
            var option = {
                ...
                tooltip: {
                    ...
                    formatter: conf['tooltipFormatter'] || "{b0} <br/>{a0}：{c0} <br/>{a1}：{c1}"    //{a}（系列名称），{b}（类目值），{c}（数值）, {d}（无）
                },
                toolbox: {
                    show: true,
                    feature: {
        ...
        initLineStackTotal: function (conf, param) { //绘制总的队伍进度图
            if (!(typeof conf == 'object' && 'id' in conf && 'url' in conf)) {
                console.warn('初始折线图堆叠line-stack失败！');
                return;
            }
                ...
                series: [
                    {
                        ...
                        data: [{value: conf['value'], name: conf['date'] + "\n完成率"}]
                    }
                ]
            };
            return setOption(conf, option);
